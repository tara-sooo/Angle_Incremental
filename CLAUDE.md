# Claude guidance

Read [AGENTS.md](AGENTS.md) for the repository's engineering rules and verification commands.

## IDD Workflow

This project uses Issue-Driven Development (IDD) with explicit issue targets. Start with [docs/idd-workflow.md](docs/idd-workflow.md) for the cross-agent entry path and phase routing.

Before starting IDD work, open `.github/instructions/idd-overview-core.instructions.md` and `.github/instructions/idd-experience.instructions.md`. Use the experience instruction to load only repository experience relevant to the current Issue, then open the routed phase file manually when the current step changes.