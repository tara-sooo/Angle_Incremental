...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `angle-incremental-blocked-by` marker, both resolved the
...
NOT contain a `angle-incremental-roadmap-id` marker (not itself
a roadmap) or a `angle-incremental-blocked-by` marker, AND
...
`angle-incremental-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`angle-incremental-roadmap-id` marker. Retro-label a legacy
...
`<!-- angle-incremental-roadmap-id: ... -->` marker are
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `angle-incremental-blocked-by` markers on the explicit target.
...
- **A0-O only** (when `issue-scope` is `orphan-first`, or when
  ...
  `angle-incremental-roadmap-id` or
  `angle-incremental-blocked-by` markers.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `angle-incremental-roadmap-id` marker when checking
  `angle-incremental-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- angle-incremental-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- angle-incremental-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- angle-incremental-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- angle-incremental-effort: S|M|L -->` footer
...
- **Roadmap identity** (`angle-incremental-roadmap-id`): placed in
...
- **Sequential dependency** (`angle-incremental-blocked-by`): placed
...
**Do not use `angle-incremental-blocked-by` to group sub-tasks under
