...
この追加スケーリングは、現在のCore Boost周回内でGenerationを実行すると緩和される。
...
報酬は、その世代中に獲得した累計スコアから次のように決まる。

```text
世代深度 = max(0, log10(今回の世代スコア / 1,000,000))
浅いGRスコア補正 = 0.60 × (1 - exp(-世代深度 / 4)) × exp(-世代深度 / 90)
浅いGRコスト補正 = 0.13 × (1 - exp(-世代深度 / 5)) × exp(-世代深度 / 80)
