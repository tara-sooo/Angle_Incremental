...
IA通常強化はIPで購入し、The Angleの通常強化と同じ価格比率・成長率を、IA側のレベルだけで独立に計算する。IAの初回価格は解放コストと同じIP帯へ移す。

| 強化 | 基礎コスト | 成長率 | 効果 |
| --- | ---: | ---: | --- |
| 周回速度 | `1.00e20 IP` | `x1.55` | IAの周回速度レベル +1 |
| 角の追加 | `2.40e20 IP` | `x1.72` | IAの頂点数 +1 |
| 頂点獲得量 | `3.60e20 IP` | `x1.68` | IAの頂点通過ごとの増加レベル +1 |
