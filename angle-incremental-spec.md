...
追加ソフトキャップ:
  通常ソフトキャップ後の log10(実効ラップスピード倍率) が 42 を超える場合、
  超過分にはさらに log10 をかける。

  実効log = 42 + log10(1 + 通常ソフトキャップ後log - 42) x 1
```
...
スコア倍率獲得係数 = 1 + min(4.5, 0.65 + log10(1 + 世代深度) x 0.90)
コスト軽減率 = min(0.18, 0.03 + log10(1 + 世代深度) x 0.032)
...
| IC | 縛り | 報酬 |
| --- | --- | --- |
| IC1 改悪された計算式 | 基礎獲得式 `(x/y)^z` の `y` が2倍される。 | `y` が撤廃される。 |
| IC2 現実的に書ける範囲で | 角の数は500を超えない。 | 通常upgrade購入スコアが ^0.95 される。 |
| IC3 ナメクジよりは早い | ラップスピードが ^0.7 され、周回速度upgradeのコスト増加が3倍になる。 | ラップスピード x1.1。 |
| IC4 うん、それ以上もそれ以下もないよ | 頂点獲得量が ^0.8 される。 | 頂点獲得量が ^1.1 される。 |
| IC5 環境配慮 | Core Boost は使えない。 | Core Boost の獲得指数 +0.01。 |
| IC6 最初だけ強い | 頂点通過ごとの増加は0.1で固定される。 | Infinity 獲得量 x2。 |
| IC7 倹約家もどき | スコアが1e100を超えると通常upgradeを購入できない。 | 通常upgrade購入時にスコアを消費しない。 |
| IC8 リアル・タイム・アタック | 角の数は100で始まり、3秒ごとに角の数が1減り、角追加upgradeは購入できない。 | 角の数はCore Boostでリセットされない。 |
