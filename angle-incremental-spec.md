...
ゲームはサーバー時刻を取得できた場合、保存データの `serverSavedAt` と現在のサーバー時刻との差を離席時間として扱う。サーバー時刻を取得できない場合は `savedAt` とローカル時刻へフォールバックする。離席報酬として処理する時間は最大7日間で、時計の逆行や保存時刻との不整合を検出した場合は報酬を付与しない。オフライン進行は復帰時に既存のゲーム更新処理を指定回数の粗いティックへ分けて実行する。ティック数は500〜1,000,000の範囲で変更でき、1ティックあたりの処理対象時間は最大24時間とする。ブラウザを長時間ブロックしないため、1回の復帰処理で実行するティック数には10,000回の安全上限を設け、これを超える指定はより粗いティックへまとめる。
...
0.9.0時点では、`APP_VERSION = 0.9.0`、`SAVE_VERSION = 10` である。Tower階数、セーブ復旧チェックポイント、チャレンジタイマー、IC/TC最速記録は既存セーブにない場合に安全な初期値として読み込み、保存形式のバージョンは変更しない。離席報酬の時刻基準はサーバー時刻を優先し、取得できない場合のみローカル時刻へフォールバックする。
