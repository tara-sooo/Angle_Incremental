...
IC8クリア後またはIC8中は、Generationで頂点数をリセットしない。
...
Core Boost はGeneration以下の進行をリセットする。Core Boost回数とCore Boost由来の補正は保持する。IC8クリア後またはIC8中は、Core Boostで頂点数をリセットしない。
...
| IC8 反出生主義 | 頂点数は3で始まり角追加を購入できず、頂点数はGR/CBでリセットされない。 | GRスコア倍率式が変化し、GRスコア倍率/100がIP獲得倍率にも適用される。 |
