...
「頂点通過ごとの増加」の表示は、1000未満では小数3桁表示を維持し、1000以上では現在の数値表記設定に従う共通フォーマッタを使う。この扱いは表示だけに適用し、計算式や保存形式には影響しない。
...
| 14-1 ペナルティは遅れてやってくる | `1e80` | 13-1 | IU11-2のInfinity回数ハードキャップを10000から30000へ延長する。 |
...
TC1〜TC4はそれぞれFloor 3、5、8、12で解放される。TC1〜TC3をクリアするまで対応する次の階数を建設できない。TCはInfinity Challengeと併用でき、開始・中止時にInfinity以下をリセットする。クリア済みのTCも再挑戦でき、恒久報酬は初回クリア時のみ解放される。TC1〜TC3の報酬はクリア後、通常プレイと後続TC内で有効になる。TC4は通常強化・IA強化をレベル1に制限し、専用強化を購入して`1e7777 Score`に到達する。TC4クリアは現在のEternity周回の条件として扱い、EternityでTC進行とともにリセットされる。
...
| TC4 | 通常強化・IA強化はレベル1まで。A/B/C専用強化を購入する | `1e7777 Score` | A: `parts + 0.85 × L_A × (1 + log2(L_A + 1))`、B: Infinity Score由来の頂点獲得指数に`0.35 × L_B`を加算、C: 効果計算のみ`+1 × L_C`無料CB |
...
オフライン進行は復帰時に既存のゲーム更新処理を指定回数の粗いティックへ分けて実行する。ティック数は1,000〜1,000,000の範囲で設定でき、指定した範囲内のティックをすべて処理する。処理は64ティックの初回プローブから開始し、各バッチの実測ticks/msを75:25の移動平均で平滑化して、約2ms分のバッチへ適応させる。バッチサイズは前回の半分〜2倍の範囲で変化し、累積約8msごとにイベントループへ処理を返す。進捗レポートのDOM更新は約100msごとと完了時に行うため、100万ティックでもブラウザを長時間ブロックせず、指定回数のティックを正確に処理できる。単調時計が利用できない場合や逆行した場合も安全にイベントループへ戻る。1ティックあたり24時間という値は長時間集約へ切り替える目安であり、処理時間の上限ではない。

Settingsの「オフライン進行」で、離席中の進行を有効／無効に切り替えられる。既存セーブに `offlineProgressEnabled` がない場合は `true` を初期値として扱い、明示的な `false` は保存・読み込み後も保持する。有効時は離席時間をゲーム更新処理へ分割して適用するため、ゲーム時間・Infinity周回時間・ゲーム内進行・自動化が進む。ただし、離席時間は実プレイ時間には含めない。無効時は離席時間をシミュレーションせず、ゲーム進行・自動化・ゲーム時間を進めずに基準時刻だけ現在時刻へ更新する。無効期間は再有効化後にも遡って処理しない。有効時の復帰では処理時間、ティック数、Infinity増加、IPをオフライン進行レポートに表示する。
...
通常のInfinity周回では、Infinity回数の増加量とオンライン周回時間からInfinity回数/秒を計算し、最高値を `bestInfinityCountPerSecond` に保存する。チャレンジ中の周回は記録対象外とする。オフライン復帰開始時点で自動化全体、Infinity自動化、最小IPしきい値、IU8-1が有効で、チャレンジ中でない場合は、保存済み最高速度を使って不足分のInfinity回数を直接集約する。集約では通常シミュレーション分を二重加算せず、小数部分は `infinityCountRateRemainder` に繰り越す。集約はInfinityポイント、履歴、周回リセットを発生させない。

### Time Flux（休止中）

Time Fluxのゲーム内効果とUIは一時的に削除している。オンライン速度は常にx1で、オフライン進行をTFへ変換する処理も行わない。将来の再導入に備え、過去のセーブに含まれるTF量、速度、任意速度、強化レベルは読み込み・保存時に保持するが、現行版では値の上限・下限によるゲームプレイ上の補正を行わない。

休止中の項目はSAVE_VERSIONを上げずに互換保持する。Time Fluxの再導入時は、UI、オンライン速度、オフライン変換、旧値の扱いを改めて設計する。
...
統計タブは概要とチャレンジ記録のサブタブに分かれる。概要では、通常進行として積算したゲーム時間と、オンライン中の実プレイ時間の統計を併記する。総プレイ時間、現在のInfinity周回時間、最速Infinity時間はゲーム時間として従来どおり保持し、それぞれに実プレイ時間の値を追加する。過去10回のInfinity履歴にもゲーム時間と実プレイ時間、到達スコア、獲得IP、挑戦中ICを記録する。旧セーブに実プレイ時間がない履歴は不明として表示する。Time Flux休止前に作られた履歴には、旧速度設定の影響が残る場合がある。
