...
オフライン進行は復帰時に既存のゲーム更新処理を指定回数の粗いティックへ分けて実行する。ティック数は1,000〜1,000,000の範囲で設定でき、指定した範囲内のティックをすべて処理する。1回のチャンクは最大1,000ティックか約8msで区切り、チャンク間でイベントループへ処理を返す。これにより、100万ティックの設定でもブラウザを長時間ブロックせず、進捗レポートを更新できる。1ティックあたり24時間という値は長時間集約へ切り替える目安であり、処理時間の上限ではない。
...
0.10.2では、`APP_VERSION = 0.10.2`、`SAVE_VERSION = 10` とする。最大1,000,000ティックのオフライン進行を非同期チャンクで処理し、条件を満たす自動Infinityの高速周回を保存済み最高速度から集約補正する。複数タブのセーブ競合では、チェックポイント退避、共有セーブの更新検知、シミュレーションと保存の停止、最新セーブへの復旧を行い、退避に失敗した場合はセーブコード出力などの復旧操作を残す。既存セーブの形式、Time Flux関連の休眠フィールド、オフライン進行のサーバー時刻優先とローカル時刻フォールバックは維持する。
