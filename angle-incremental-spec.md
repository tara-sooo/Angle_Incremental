...
IC8中は、Generationで頂点数をリセットしない。IC8中に購入した角追加アップグレードは角を追加せず、IC8中専用の補正レベルとして保持する。
...
Core Boost はGeneration以下の進行をリセットする。Core Boost回数とCore Boost由来の補正は保持する。IC8中は、Core Boostで頂点数をリセットしない。
...
| IC8 反出生主義 | 頂点数は3で固定され、CB必要スコアは `^2` される。角追加アップグレードは角を追加せず、頂点通過ごとの増加とスコア獲得量指数を上げる。 | GRスコア倍率式が変化し、GRスコア倍率/100がIP獲得倍率にも適用される。 |
