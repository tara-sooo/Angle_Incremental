...
0.10.1では、`APP_VERSION = 0.10.1`、`SAVE_VERSION = 10` とする。タッチ端末向けのボタン押下表示とオフライン進行設定を追加し、リリース時のバージョン表記整合性検査を導入する。Tower階数、セーブ復旧チェックポイント、チャレンジタイマー、IC/TC最速記録、高位実績マスクは既存セーブにない場合に安全な初期値として読み込み、保存形式のバージョンは変更しない。Time Flux関連の休眠フィールドも既存セーブ互換性のため保持する。離席報酬の時刻基準はサーバー時刻を優先し、取得できない場合のみローカル時刻へフォールバックする。
