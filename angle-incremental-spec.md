...
ラップスピード倍率が ×200 を超える場合、実効倍率にはソフトキャップを適用する。

```text
実効ラップスピード倍率 = 200 × (生ラップスピード倍率 / 200)^0.5
```

これにより、ラップスピード強化だけで Core Boost まで到達しにくくする。
...
スコア獲得量は、一定の頂点数を超えると核到達時の獲得量に追加式がかかる。

核到達時の通常獲得量を `x`、現在の頂点数から決まる項数を `y` とする。

```text
獲得スコア = (x / y) ^ y
```

この式は表示だけでなく、実際のスコア獲得量に適用される。
...
Generation スコア倍率が伸びると、ラップスピードのソフトキャップ開始点が少し上がる。

```text
ソフトキャップ開始点 = 200 x (1 + min(1.5, log10(Generationスコア倍率) x 0.15))
```

IC1中は Generation が機能しないため、この緩和も無効になる。
