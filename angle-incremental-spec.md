...
Generationコスト倍率 = max(0.9, Generationコスト倍率 x (1 - コスト軽減率))
```
...
既存セーブで Generationコスト倍率 が 0.9 未満の場合は、0.9へ丸める。
...
| 1-2 はじめてのQoL | 1 | 1-1 | 通常強化の自動購入を解放する。 |
| 2-1 最速タイム | 1 | 1-1, 1-2 | ラップスピードが x1.5 される。 |
| 3-1 実績3と7を獲得するにはこれがほぼ必須です | 3 | 2-1 | GR強化項目のスコア倍率指数を x1.5 する。 |
