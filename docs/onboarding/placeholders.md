...
| -------------------------------- | --------------------------------------------------------- | ---------------------------------- |
| `Angle_Incremental`                  | Repository short name used in worktree examples           | `my-app`                           |
| `angle-incremental`      | Hidden issue-body marker prefix                           | `my-app`                           |
| `tara-sooo`       | Single JSON-escaped login allowed to post trusted markers | `trusted-user-a`                   |
| `npm run check:runtime-order && npm run check:syntax`      | Auto-fix plus validate command row                        | `npm run lint:fix && npm run lint` |
| `npm run validate` | Non-mutating verify command row                           | `npm run lint && npm run test`     |
| `npm run validate` | Post-fix validate command row                             | `npm run lint:fix && npm test`     |
| `npm ci`       | Dependency install command, or `true` when unnecessary    | `npm install`                      |
...
- no relevant auto-fix command →
  `npm run check:runtime-order && npm run check:syntax = true`

Keep `npm ci` safe to rerun across retries, takeovers,
and recreated worktrees.

## Marker prefix notes

`angle-incremental` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- angle-incremental-roadmap-id: {unique-id} -->`
- blocked-by marker:
  `<!-- angle-incremental-blocked-by: {roadmap-id} -->`
