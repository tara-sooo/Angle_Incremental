...
| ------------------------------------------------------------------- | -------------------------------- |
| `idd-skill` in repo-name contexts                                   | `Angle_Incremental`                  |
| `idd-skill` in marker-prefix contexts (e.g. `idd-skill-roadmap-id`) | `angle-incremental`      |
| **fix-validate** command string                                     | `npm run check:runtime-order && npm run check:syntax`      |
| **pre-push-validate** command string                                | `npm run validate` |
| **post-fix-validate** command string                                | `npm run validate` |
| **install-deps** command string                                     | `npm ci`       |
...
- `audit-docs.mjs` enforces synchronization rules defined in
  `sync-manifest.json`
- Template copies use placeholders like `Angle_Incremental` to support
...
IDD's frequent **next-into-feature re-sync cadence** is the dominant driver of
...
- Remember that **re-sync cadence is the main cost lever**: the
  merge-from-`next` freshness model trades CI minutes for merge safety, so tune
