...
function currentLog(runtime, key) { return runtime.currentLog10ForValue(runtime.state[key], runtime.state[`${key}Log10`]); }
function installPreInfinityAggregateBatch(runtime) {
  runtime.processManyVertices = function(start, end) {
    ...
    const events = runtime.coreVertexIndices().map((coreIndex) => {
    ...
    if (events.length) {
      let earned = 0; let totalHits = 0; let lastStep = 0;
      ...
      const fallbackLog = runtime.log10Value(Math.max(totalHits, 1)) + runtime.finalScoreGainFromBaseLog10(runtime.gainAfterIncreaseLog10(increase, lastStep));
...
function policyDepth(policy, cb, gr) {
  if (policy.kind === "fixed") return policy.depth;
  if (policy.kind === "by-core") return policy.depths[Math.min(cb, policy.depths.length - 1)];
  if (policy.kind === "late-split") return cb < 4 ? policy.baseDepth : (gr < policy.switchAfter ? policy.earlyDepth : policy.lateDepth);
  throw new Error(`Unknown policy kind: ${policy.kind}`);
}
function allowGeneration(policy, cb, gr) { return policy.maxGenerationAfterCore4 === undefined || cb < 4 || gr < policy.maxGenerationAfterCore4; }
async function simulate(policy, cutoffSeconds = BASELINE_CUTOFF_SECONDS) {
  ...
  state.showFloatingText = false; state.lightEffects = true;
  runtime.saveGame = () => {}; runtime.updateUi = () => {};
  installPreInfinityAggregateBatch(runtime);
  let time = 0, nextAction = 0, buyAllCalls = 0, generations = 0, coreBoosts = 0;
  const resetTimeline = [];
  while (time < cutoffSeconds && state.infinityCount === 0) {
    ...
    if (dt > 0) { debug.update(dt); time = nextTime; }
    if (time + 1e-9 >= nextAction) {
      buyByMode(debug, policy.purchaseMode || "all"); buyAllCalls += 1;
      ...
      const depth = policyDepth(policy, state.coreBoostCount, state.generationCount);
      const generationScoreLog = currentLog(runtime, "generationScore");
      const canCore = runtime.canCoreBoost() && scoreLog >= coreTarget;
      const canGeneration = runtime.canRunGeneration() && allowGeneration(policy, state.coreBoostCount, state.generationCount) && generationScoreLog >= generationRequirement + depth;
      if (policy.priority === "generation" && canGeneration) {
        debug.runGeneration(); generations += 1;
        resetTimeline.push({ type: "GR", time, coreBoostCount: state.coreBoostCount, generationCountBefore: state.generationCount - 1, requirementLog10: generationRequirement, scoreLog10: generationScoreLog, depth });
      } else if (canCore) {
        debug.runCoreBoost(); coreBoosts += 1;
  ...
  return { name: policy.name, policy, reachedInfinity: state.infinityCount > 0, firstInfinitySeconds: infinityRun ? infinityRun.time : null, elapsedSeconds: time, finalScoreLog10: runtime.currentScoreLog10(), buyAllCalls, generations, coreBoosts, resetTimeline };
}
function fixed(name, depth, extra = {}) { return { name, kind: "fixed", depth, purchaseMode: "all", coreDelayLog: 0, priority: "core", ...extra }; }
function createCandidates() {
  ...
  for (const stage of [1,2,3,4]) for (const depth of [12,16,24,30,40]) { const depths=[20,20,20,20,20]; depths[stage]=depth; candidates.push({name:`by-core_stage${stage}_d${depth}`,kind:"by-core",depths,purchaseMode:"all",coreDelayLog:0,priority:"core"}); }
  for (const switchAfter of [1,2]) for (const earlyDepth of [15,20]) for (const lateDepth of [30,40,60]) candidates.push({name:`late-split_after${switchAfter}_early${earlyDepth}_late${lateDepth}`,kind:"late-split",baseDepth:20,switchAfter,earlyDepth,lateDepth,purchaseMode:"all",coreDelayLog:0,priority:"core"});
  return candidates;
}
async function runFirstInfinityStrategySearchAudit() {
  const candidates = createCandidates(); const results=[];
  for (const candidate of candidates) results.push(await simulate(candidate));
  const winners=results.filter((r)=>r.reachedInfinity).sort((a,b)=>a.firstInfinitySeconds-b.firstInfinitySeconds);
  const report={assumptions:{newSave:true,infinityUpgradesOrAutomation:false,buyAllIntervalSeconds:ACTION_INTERVAL_SECONDS,baselineCutoffSeconds:BASELINE_CUTOFF_SECONDS,coreBoostPolicy:"Core Boost is immediate unless a candidate explicitly adds a log-delay.",generationPolicy:"Generation occurs when Generation Score reaches the policy's requirement + depth threshold.",candidateCount:candidates.length,candidateFamilies:["fixed depth","Core Boost delay","selective purchases","post-CB4 Generation limit","per-Core-Boost depth","late-phase split depth"],persistence:"Disabled only in the audit runtime; it does not affect game state."},reference:results.find((r)=>r.name==="reference_fixed_d20"),best:winners[0]||null,winners,nonWinners:results.filter((r)=>!r.reachedInfinity)};
  fs.writeFileSync(reportPath,`${JSON.stringify(report,null,2)}\n`);
  console.log("FIRST_INFINITY_STRATEGY_SEARCH",JSON.stringify({candidates:candidates.length,referenceSeconds:report.reference&&report.reference.firstInfinitySeconds,best:report.best&&{name:report.best.name,seconds:report.best.firstInfinitySeconds,generations:report.best.generations,coreBoosts:report.best.coreBoosts},beatingReference:winners.filter((r)=>r.firstInfinitySeconds<BASELINE_CUTOFF_SECONDS).length}));
  return report;
}
