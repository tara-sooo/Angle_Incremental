...
const runtimeOrder = require("./candidate-runtime-order.js");

const repositoryRoot = path.resolve(__dirname, "..");
const candidateRuntimePath = path.resolve(repositoryRoot, "src", "main.js");
const candidatePreludePaths = runtimeOrder
  .slice(0, -1)
...
function evaluateClassicScript(context, filePath) {
  const source = fs.readFileSync(filePath, "utf8");
  vm.runInContext(source, context, { filename: path.basename(filePath) });
}

function loadRuntime(runtimePath, initialStorage) {
  const { context, storage } = createContext(initialStorage);
  vm.createContext(context);
  if (path.resolve(runtimePath) === candidateRuntimePath) {
    candidatePreludePaths
      .filter((filePath) => fs.existsSync(filePath))
      .forEach((filePath) => evaluateClassicScript(context, filePath));
  }
  evaluateClassicScript(context, runtimePath);
...
function stableValue(value) {
  ...
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, stableValue(entry)]));
  return value;
}
