...
const { loadRuntime, snapshot } = require("./runtime-harness.js");

const baselinePath = path.join(__dirname, "fixtures", "next-runtime.js");
const candidatePath = path.join(__dirname, "..", "src", "main.js");

function compareScenario(name, act) {
  const baseline = loadRuntime(baselinePath);
  const candidate = loadRuntime(candidatePath);
  act(baseline);
  act(candidate);
  assert.deepStrictEqual(snapshot(candidate), snapshot(baseline), `${name}: candidate runtime diverged from next baseline`);
}
...
function makeStoredSave(runtimePath) {
  const instance = loadRuntime(runtimePath);
...
async function runDifferentialTests() {
  compareScenario("initial state and rendering", () => {});

  compareScenario("normal upgrades and medium-speed simulation", ({ debug }) => {
    const { state } = debug;
  ...
  compareScenario("Generation and Core Boost reset sequence", ({ debug }) => {
    const { state } = debug;
  ...
  compareScenario("IU5-2 reset start score and IU6-2 floor", ({ debug }) => {
    const { state } = debug;
  ...
  compareScenario("Infinity Challenge 6 reward", ({ debug }) => {
    const { state } = debug;
    state.infinityCount = 1;
  ...
  compareScenario("Infinity Challenge 7 affordability and reward", ({ debug }) => {
    const { state } = debug;
  ...
  compareScenario("Infinity Challenge 8 active preservation", ({ context, debug }) => {
    const { state } = debug;
    state.infinityCount = 1;
  ...
  compareScenario("high-speed vertex processing", ({ debug }) => {
    const { state } = debug;
  ...
  {
    const baselineSave = makeStoredSave(baselinePath);
    const baselineLoaded = loadRuntime(baselinePath, baselineSave);
    const candidateLoaded = loadRuntime(candidatePath, baselineSave);
    assert.deepStrictEqual(snapshot(candidateLoaded), snapshot(baselineLoaded), "legacy local save must load identically in candidate runtime");
  }

  {
    const baselineSource = loadRuntime(baselinePath);
    seedLateGameState(baselineSource);
    const baselineCode = await baselineSource.debug.exportSaveCode();

    const baselineImported = loadRuntime(baselinePath);
    const candidateImported = loadRuntime(candidatePath);
    ...
    assert.deepStrictEqual(snapshot(candidateImported), snapshot(baselineImported), "candidate must restore a next save code identically");

    const candidateCode = await candidateImported.debug.exportSaveCode();
    const baselineReloaded = loadRuntime(baselinePath);
    ...
    assert.deepStrictEqual(snapshot(baselineReloaded), snapshot(candidateImported), "candidate save code must remain backward-compatible");
  }

  console.log("differential runtime tests passed");
}
