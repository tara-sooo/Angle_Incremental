...
async function compareScenario(name, act) {
  ...
  assert.deepStrictEqual(
    ...
    `${name}: candidate runtime diverged from next baseline outside the approved release differences`,
  );
}
...
async function runDifferentialTests() {
...

  {
    ...
    assert.deepStrictEqual(
      ...
      "legacy local save must load identically outside the approved release differences",
    );
  }

  {
    ...
    assert.deepStrictEqual(
      ...
      "candidate must restore a next save code identically outside the approved release differences",
    );
    ...
    assert.deepStrictEqual(
      ...
      compatibilitySnapshot(candidateImported),
      "candidate save code must remain backward-compatible outside the approved release differences",
    );
  }
